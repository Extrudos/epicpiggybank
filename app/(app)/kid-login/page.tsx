...
export default function KidLoginPage() {
  ...
  useEffect(() => {
    fetch("/api/kids")
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data)) setKids(data.filter((k: Kid & { is_active: boolean }) => k.is_active));
      })
  ...
  if (selectedKid) {
    return (
      <div className="flex flex-col items-center py-12" data-mode="kid">
        ...
        <div className="w-20 h-20 rounded-full bg-kid-coral/20 flex items-center justify-center text-3xl mb-4">
          {selectedKid.avatar_url || selectedKid.display_name.charAt(0).toUpperCase()}
        </div>
        <h2
        ...
        <Card className="card-playful max-w-sm">
          <CardContent className="py-10 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              No kids added yet. Ask a parent to add you!
            </p>
          </CardContent>
        </Card>
      ) : (
